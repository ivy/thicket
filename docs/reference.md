...
| Voice Insights (`insights.twilio.com/v1/Voice/{CallSid}/Summary`, `/Events`, `/Metrics`) answers 404 with an empty body for every spike call, with the restricted key and with the auth token alike, 15+ minutes after the call: "Voice Insights Advanced Features must be active to use this API Resource" — a Console toggle, not a permission. A partial summary is documented at ~10 min after a call, the complete one at ~30 min | observed 2026-08-30; https://www.twilio.com/docs/voice/voice-insights/api/call/call-summary-resource |
