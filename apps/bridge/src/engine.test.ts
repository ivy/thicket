...
test("a follow-up mid-block closes the block before the fresh message opens", async () => {
  const r = rig({
    script: folding(() => [
      taskEvent("t1", "ctx"),
      artifactEvent("t1", "```ts\nconst a = 1;\n", false, false),
  ...
  assert.ok(
    messages[1]!.startsWith("```ts\n"),
    `the message after the cut reopens the block: ${JSON.stringify(messages[1])}`,
  );
  r.state.close();
});
